...
public class CommentsResourceImpl extends XWikiResource implements CommentsResource
{
            ...
            throws XWikiRestException
    {
        try {
            ...
            // V&V: un comentario sin texto ni highlight es invalido. Antes XWiki lo aceptaba en
            // silencio (devolvia null sin guardar). Ahora se rechaza con 400 Bad Request.
            boolean emptyText = comment.getText() == null || comment.getText().trim().isEmpty();
            boolean emptyHighlight = comment.getHighlight() == null || comment.getHighlight().trim().isEmpty();
            if (emptyText && emptyHighlight) {
